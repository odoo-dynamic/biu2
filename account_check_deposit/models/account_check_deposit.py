...
class AccountCheckDeposit(models.Model):
    ...
    def _compute_check_deposit(self):
        rg_res = self.env["account.move.line"].read_group(
            ...
            ["check_deposit_id", "debit","credit", "amount_currency"],
            ["check_deposit_id"],
        )
        mapped_data = {
        ...
        for deposit in self:
            ...
            if company_cur != deposit.currency_id:
                ...
                if deposit.type_deposit=='customer':
                    total = mapped_data.get(deposit.id, {"debit": 0.0})["debit"]
                elif deposit.type_deposit=='supplier':
                    total = mapped_data.get(deposit.id, {"credit": 0.0})["credit"]
    ...
    check_payment2_ids = fields.One2many(
        comodel_name="account.move.line",
        inverse_name="check_deposit_id",
        string="Check Payments",
        readonly=True,
    ...
    state = fields.Selection(
        selection=[("draft", "Draft"),("done", "Done"), ("cancel","Canceled")],
        string="Status",
        default="draft",
        readonly=True,
        tracking=True,
    ...
    move_cancel_id = fields.Many2one(
        'account.move', string='Journal Entry Cancel', readonly=True)
        ...
        for deposit in self:
            if deposit.type_deposit=='customer':
                if deposit.move_id:
                    ...
                    header_joure = {'journal_id': self.bank_journal_id.id,
                                    'date':self.deposit_date,
                                    'ref': 'Check Deposit ' + self.name,
                    ...
                    for item in range(0, 2):
                        if item == 0:
                            for line in self.check_payment_ids:
                                rec_item = {'account_id': self.journal_id.default_account_id.id,
                                            'partner_id': line.partner_id.id,
                                            'name': 'Check Deposit - Ref. Check ' + line.ref,
                                            'debit': line.debit,
                                            'credit': 0,
                                            'currency_id': line.currency_id.id or False,
                                            'amount_currency': line.amount_currency,
                                            'check_deposit_id':self.id,
                                            'deposit_no':line.deposit_no,
                                            'deposit_due':line.deposit_due
                                            }
                                total_amount+=line.debit
                                total_currency+=line.amount_currency
                                add_item.append((0, 0, rec_item))
                                line.write({'check_deposit_id':False})
                        else:
                            rec_item = {'account_id': self.bank_journal_id.default_account_id.id,
                                        'name': 'Check Deposit ' + self.name,
                                        'debit': 0,
                                        'credit': total_amount,
                                        'currency_id': self.currency_id.id,
                                        'amount_currency': abs(total_currency)*-1,
                                        }
                            add_item.append((0, 0, rec_item))
                    header_joure['line_ids'] = add_item

                    joure_post = self.env['account.move'].create(header_joure)
                    joure_post.post()

                    # deposit.move_id.unlink()
                    deposit.write({'state': 'cancel', 'move_cancel_id': joure_post.id})
            elif deposit.type_deposit=='supplier':
                if deposit.move_id:
                    # It will raise here if journal_id.update_posted = False
                    # deposit.move_id.button_cancel()
                    for line in deposit.check_payment2_ids:
                        if line.reconciled:
                            line.remove_move_reconcile()
                    ...
                    for item in range(0, 2):
                        if item == 0:
                            for line in self.check_payment2_ids:
                                rec_item = {'account_id': self.journal_id.default_account_id.id,
                                            'partner_id': line.partner_id.id,
                                            'name': 'Check Deposit - Ref. Check ' + line.ref,
                                            'debit': 0,
                            ...
                            rec_item = {'account_id': self.bank_journal_id.default_account_id.id,
                                        ...
                                        'credit': 0,
                                        'currency_id': self.currency_id.id,
                                        'amount_currency': abs(total_currency),
                                        }
                            add_item.append((0, 0, rec_item))
                    header_joure['line_ids'] = add_item
    ...
    def _prepare_supplier_move_line_vals(self, line):
    ...
    def _prepare_counterpart_supplier_move_lines_vals(self, total_debit, total_amount_currency):
        self.ensure_one()
        if not self.bank_journal_id.payment_debit_account_id:
            raise UserError(
                _("Missing 'Outstanding Receipts Account' on the bank journal '%s'.")
        ...
        for deposit in self:
            if deposit.type_deposit=='customer':
                ...
                for line in deposit.check_payment_ids:
                    ...
                    total_amount_currency += line.amount_currency
                    line_vals = self._prepare_supplier_move_line_vals(line)
                    line_vals["move_id"] = move.id
                    move_line = move_line_obj.with_context(
                        check_move_validity=False
                    ).create(line_vals)
                ...
                # Create counter-part
                counter_vals = deposit._prepare_counterpart_supplier_move_lines_vals(
    ...
    def get_all_checks(self):
        ...
        elif self.type_deposit=='supplier':
            all_pending_checks = self.env["account.move.line"].search(
                [
                    ("company_id", "=", self.company_id.id),
                    ("reconciled", "=", False),
