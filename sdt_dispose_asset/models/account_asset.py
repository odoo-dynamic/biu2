    ...
    def _compute_value_residual(self):
        for record in self:
            if record.disposal_date:
                record.value_residual = record.original_value - record.salvage_value - abs(sum(record.depreciation_move_ids.filtered(lambda m: m.state == 'posted' and m.auto_post==False).mapped('amount_total')))
            else:
